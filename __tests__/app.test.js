	...
	test("PATCH 200: updates article votes by id", () => {
		...
		return request(app)
			...
			.then(({ body }) => {
				...
				expect(patchedArticle.title).toBe(
...
describe("api/articles", () => {
	...
	test("GET 200: array is filtered by topic query", () => {
		return request(app)
			...
			.then(({ body }) => {
				...
				expect(responseArticlesArray).toBeSortedBy("created_at", {
					descending: true,
				});
			});
	});
	...
	test("GET 404: returns a 404 error for a route that does not exist", () => {
		return request(app).get("/api/notarticles").expect(404);
	});
...
describe("api/users", () => {
	...
	test("GET 404: returns a 404 error for a route that does not exist", () => {
		return request(app).get("/api/nonsenseRoute").expect(404);
	});
});
